...
def connect_and_find_user(userid_str: str, dataset_hint=None):
    """
    Try every configured MongoDB URI.
    Try ObjectId form and plain-string form of userid.

    If `dataset_hint` (a databaseid string, or list of them) is given, a server
    is only accepted if it actually has ecg_dataouts beats for THAT SPECIFIC
    dataset — not just any data for the userid. This matters because a user's
    data can be split across servers (e.g. older sessions on web1, newer ones
    on web2); without this check, connect_and_find_user can lock onto the
    wrong server (one with *some* unrelated data for this userid) and every
    subsequent dataset lookup silently returns zero beats.

    If `dataset_hint` is None (e.g. for --dataset all), falls back to the
    original behavior: accept the first server with ANY data for this userid.

    Return (db_handle, uid_value) for the first connection that qualifies.
    """
    candidates = []
    ...
    if dataset_hint is None:
        hints = None
    elif isinstance(dataset_hint, (list, tuple, set)):
        hints = [str(d) for d in dataset_hint]
    else:
        hints = [str(dataset_hint)]

    # Pass 1: if we have specific dataset(s) to look for, require an exact
    # match in ecg_dataouts before accepting a server.
    if hints:
        for name, uri in MONGO_URIS.items():
            try:
                ...
                for uid_val in candidates:
                    n = db["ecg_dataouts"].count_documents(
                        {"userid": uid_val, "databaseid": {"$in": hints}}, limit=1
                    )
                    if n > 0:
                        print(f"  Connected : {name}  (userid as {type(uid_val).__name__}, "
                              f"verified dataset match)")
                        return db, uid_val
            except Exception as e:
                print(f"  [{name}] unreachable: {e}")
                continue
        print(f"  No server has ecg_dataouts beats for dataset(s)={hints} under this userid.")
        print(f"  Falling back to 'any data for this user' search (results may be empty for this dataset) ...")

    # Pass 2 (or only pass, if no dataset_hint): original behavior — first
    # server with ANY data for this userid, in any of these collections.
    for name, uri in MONGO_URIS.items():
        try:
            client = MongoClient(uri, serverSelectionTimeoutMS=6000)
            client.admin.command("ping")
            ...
            for uid_val in candidates:
                for col in ["ecg_dataouts", "ecg_value_mins", "graph_marker"]:
                    ...
                    if n > 0:
                        print(f"  Connected : {name}  (userid as {type(uid_val).__name__}, "
                              f"unverified — no exact dataset match found on any server)")
                        return db, uid_val
        except Exception as e:
            print(f"  [{name}] unreachable: {e}")
            continue
...
def _ncc(seg, tpl):
    """
    Normalized cross-correlation coefficient at zero lag, bounded in
    [-1, 1] -- this is what NCC_THRESHOLD is meant to compare against.

    NOTE: a previous version of this function omitted the division by N,
    so it returned correlate(z-normed seg, z-normed tpl) directly -- which
    is actually N * (true correlation coefficient), not the coefficient
    itself. That meant any segment with even weak positive correlation
    (e.g. true r ~ 0.15) could produce a raw score in the tens, trivially
    clearing a 0.75 threshold meant for a 0-1 scale, making Gate 2 nearly
    a no-op in practice. Dividing by len(seg) restores the intended
    behavior: an actual percentage-style morphology match score.
    """
    n = len(seg)
    if n == 0:
        return 0.0
    c = correlate(_znorm(seg.astype(float)), _znorm(tpl.astype(float)), mode="valid")
    return float(np.max(c)) / n if len(c) > 0 else 0.0
...
def classify_beat(seg, raw_vt, vt_bank, nsr_bank, svm, arrhythmia=None):
    ...
    Three-gate classifier:
      ...
      Gate 3 – SVM    : confirmed VT if EITHER
                          (a) P(VT) > SVM_PROB_THRESH AND sim_diff > SIM_DIFF_THRESH, OR
                          (b) P(VT) > SVM_VERY_HIGH_PROB  (high confidence alone is
                              enough, even if sim_diff is small — prevents a
                              near-certain VT call from being vetoed by the
                              secondary margin check).
    """
    ...
    # Gate 3: SVM — OR between the standard combined check and the
    # high-confidence-alone shortcut
    ...
    standard_pass = prob > SVM_PROB_THRESH and diff > SIM_DIFF_THRESH
    high_conf_pass = prob > SVM_VERY_HIGH_PROB
    if standard_pass or high_conf_pass:
...
# ═══════════════════════════════════════════════════════════════════════════════
# Importable entry point — used by analyze_weball.py right after analysis finishes
# ═══════════════════════════════════════════════════════════════════════════════

def run_vt_scan_on_datasets(userid_str, dataset_ids, update_label=None):
    """
    Run the full VT NCC+SVM scan restricted to a specific, already-known list of
    dataset_ids (databaseid values) for one user, and print VT presence per
    session plus a grand summary.

    This is the function analyze_weball.py calls right after full_analyze()
    finishes processing its datasets, so VT scanning only covers what was
    just analyzed in this run — not the user's entire history.

    Parameters
    ----------
    userid_str : str
        The user's ObjectId as a string (same value passed to vt_scan.py --userid).
    dataset_ids : list[str]
        List of databaseid / ecgvalue_id values (e.g. "2026-06-05 07:16:48")
        that were just processed by analyze_weball.py's full_analyze().
    update_label : str, optional
        Pass "vt" to tag the top-5 most-confident VT beats per session in
        MongoDB with label="vt" (same behavior as the --update_label CLI flag).

    Returns
    -------
    (all_episodes, all_isolated) : same shape as the CLI's internal accumulators
        all_episodes : list[(dbid, episode_beat_list)]
        all_isolated : list[(dbid, beat_no)]
    """
    print("\n" + "#" * 60)
    print("  VT SCAN  (post-analysis)")
    print("#" * 60)

    if not dataset_ids:
        print("  No datasets to scan — skipping VT scan.")
        return [], []

    # 1. Load templates + train SVM (own MONGO_URIS / template dirs, independent
    #    of analyze_weball's get_config db connection, by design)
    np.random.seed(42)
    try:
        vt_bank, nsr_bank, raw_vt = load_banks()
    except SystemExit as e:
        print(f"  VT scan aborted: {e}")
        return [], []

    print("\nTraining SVM ...")
    svm = train_svm(vt_bank, nsr_bank)
    print(f"Gates: NCC>={NCC_THRESHOLD}  [P(VT)>{SVM_PROB_THRESH} AND sim_diff>{SIM_DIFF_THRESH}]  "
          f"OR P(VT)>{SVM_VERY_HIGH_PROB}  min_run={MIN_VT_RUN}  max_gap={MAX_GAP}")

    # 2. Connect — try all MongoDB servers (vt_scan's own fallback logic).
    #    Pass dataset_ids as a hint so we land on the server that actually
    #    HAS these specific sessions, not just any server with old data for
    #    this userid (a user's sessions can be split across servers).
    print("\nConnecting to MongoDB (vt_scan) ...")
    db, uid_val = connect_and_find_user(userid_str, dataset_hint=dataset_ids)
    if db is None:
        print(f"  ERROR: No MongoDB connection found data for userid={userid_str}")
        print("  VT scan skipped.")
        return [], []

    dataset_ids = sorted(str(d) for d in dataset_ids)
    print(f"\nScanning {len(dataset_ids)} dataset(s) just analyzed:")
    for ds in dataset_ids:
        print(f"  {ds}")

    # 3. Process only the datasets just analyzed
    all_episodes, all_isolated = [], []
    for dbid in dataset_ids:
        eps, iso = process_session(
            db, uid_val, dbid, vt_bank, nsr_bank, raw_vt, svm, update_label
        )
        for ep in eps:  all_episodes.append((dbid, ep))
        for b  in iso:  all_isolated.append((dbid, b))

    # 4. Presence summary — explicit yes/no print, easy to grep from logs
    print(f"\n{'='*60}")
    print(f"  VT PRESENCE SUMMARY  —  {len(dataset_ids)} dataset(s) scanned")
    print(f"{'='*60}")
    if not all_episodes and not all_isolated:
        print(f"  >>> VT NOT PRESENT  (userid={userid_str})")
    else:
        print(f"  >>> VT PRESENT  (userid={userid_str})")
        if all_episodes:
            print(f"  VT episodes : {len(all_episodes)}")
            for dbid, ep in all_episodes:
                print(f"    [{dbid}]  beats {ep[0]}->{ep[-1]}  ({len(ep)} beats)")
        if all_isolated:
            print(f"  Isolated VT beats : {len(all_isolated)}")
            for dbid, b in all_isolated:
                print(f"    [{dbid}]  beat {b}")
    print("#" * 60 + "\n")

    return all_episodes, all_isolated


# ═══════════════════════════════════════════════════════════════════════════════
# CLI Main (unchanged behavior — runs full scan over chosen datasets standalone)
# ═══════════════════════════════════════════════════════════════════════════════
...
def main():
    ...
    #    If a specific dataset was given (not "all"), pass it as a hint so we
    #    verify it actually exists on the server we connect to, rather than
    #    landing on a server that merely has *some* old data for this userid.
    print("\nConnecting to MongoDB ...")
    dataset_hint = None if args.dataset.strip().lower() == "all" else args.dataset.strip()
    db, uid_val = connect_and_find_user(args.userid, dataset_hint=dataset_hint)
    if db is None:
